...
class Place {
  constructor(place) {
    ...
    const { street, city, state, zip, country } = this.extractAddressComponents()
  ...
  static async savePlacesData(places) {
    ...
    try {
      for (const place of places) {
        upsertCount += await upsertPlace(place)
      }
    } catch (error) {
      console.error(error)
    } finally {
    ...
    console.log(db.pool.totalCount, db.pool.idleCount, db.pool.waitingCount)
    console.log(`${upsertCount} places queried and inserted/updated.`)
  }
